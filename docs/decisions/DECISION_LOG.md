...
| D62 | **El registro guarda una copia del título de la producción.** Si el admin borra una ficha, el diario sigue diciendo qué vio la persona; mientras la ficha exista manda el título vivo del catálogo, y la respuesta marca con `enCatalogo` si todavía hay adónde ir | También de la revisión externa: el registro huérfano conservaba fecha, puntaje y reseña, y perdía lo único que lo hace un historial. Se descartó impedir el borrado (obliga a mover el DELETE fuera de Catálogo y traba la fusión de duplicados que D14 y D37 dan por hecha) y la baja lógica (toca todas las consultas del catálogo y agrega un estado paralelo a los de D8) |
