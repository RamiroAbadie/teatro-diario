...
| D59 | **La fecha difusa (MD-1) se guarda normalizada al comienzo de su período** (`marzo de 2023` → `2023-03-01`) junto con la granularidad. El orden del diario (MD-2) sale entonces de un `ORDER BY fecha DESC` y los registros sin fecha viajan en una lista aparte de la respuesta, no mezclados | Ni se inventan fechas falsas ni se ordena en memoria. La granularidad es lo que el frontend necesita para mostrar "marzo de 2023" y no "1 de marzo de 2023" |
